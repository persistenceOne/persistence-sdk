...
func Test_Quash_Request(t *testing.T) {
	...
	require.Equal(t, transactionRequest{BaseReq: test_types.BaseReq{From: cliContext.GetFromAddress().String(), ChainId: cliContext.ChainID, Simulate: cliContext.Simulate}, FromID: "", IdentityID: ""}, requestFromCLI)
	...
	require.Equal(t, newMessage(fromAccAddress, testBase.NewID("fromID"), testBase.NewID("identityID")), msg)
	require.Nil(t, Error)

	msg2, Error := newTransactionRequest(test_types.BaseReq{From: "randomFromAddress", ChainId: "test"}, "fromID", "identityID").MakeMsg()
	require.NotNil(t, Error)
	require.Nil(t, msg2)
