...
func Test_Deputize_Request(t *testing.T) {
	...
	require.Equal(t, nil, Error)
	require.Equal(t, transactionRequest{BaseReq: protoTypes.BaseReq{From: cliContext.GetFromAddress().String(), ChainId: cliContext.ChainID, Simulate: cliContext.Simulate}, FromID: "", ToID: "", ClassificationID: "", MaintainedProperties: "", AddMaintainer: false, RemoveMaintainer: false, MutateMaintainer: false}, requestFromCLI)
	...
	require.Equal(t, newMessage(fromAccAddress, base2.NewID("fromID"), base2.NewID("toID"), base2.NewID("classificationID"), maintainedProperties, false, false, false), msg)
	require.Nil(t, Error)

	msg2, Error := newTransactionRequest(protoTypes.BaseReq{From: "randomString", ChainId: "test", Fees: sdkTypes.NewCoins()}, "fromID", "toID", "classificationID", maintainedProperty, false, false, false).MakeMsg()
	require.NotNil(t, Error)
	require.Nil(t, msg2)
