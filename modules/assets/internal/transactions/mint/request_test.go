...
func Test_Mint_Request(t *testing.T) {
	...
	require.Equal(t, nil, Error)
	require.Equal(t, transactionRequest{BaseReq: protoTypes.BaseReq{From: cliContext.GetFromAddress().String(), ChainId: cliContext.ChainID, Simulate: cliContext.Simulate}, FromID: "", ToID: "", ClassificationID: "", ImmutableMetaProperties: "", ImmutableProperties: "", MutableMetaProperties: "", MutableProperties: ""}, requestFromCLI)
	...
	require.Equal(t, newMessage(fromAccAddress, base2.NewID("fromID"), base2.NewID("toID"), base2.NewID("classificationID"), immutableMetaProperties, immutableProperties, mutableMetaProperties, mutableProperties), msg)
	require.Nil(t, Error)

	msg, Error = newTransactionRequest(protoTypes.BaseReq{From: "randomFromAddress", ChainId: "test"}, "fromID", "toID", "classificationID", immutableMetaPropertiesString, immutablePropertiesString, mutableMetaPropertiesString, mutablePropertiesString).MakeMsg()
