...
func (simulationApplication SimulationApplication) Setup(isCheckTx bool) applications.SimulationApplication {
	...
	app := NewSimulationApplication().Initialize(ApplicationName, applications.EncodingConfig{}, wasm.EnableAllProposals, ModuleAccountPermissions, AllowedReceivingModuleAccounts, log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, 0, map[int64]bool{}, DefaultNodeHome, simapp.EmptyAppOptions{})
...
func NewSimulationApplication() SimulationApplication {
	return SimulationApplication{}
}
