...
var (
	...
	ModuleBasics = module.NewBasicManager(
		...
		bank.AppModuleBasic{},
		// capability.AppModuleBasic{}, // Removed in v0.50+
		...
		gov.NewAppModuleBasic(
			[]govclient.ProposalHandler{
				...
				// upgradeclient.LegacyProposalHandler, // Removed in v0.50+
				// upgradeclient.LegacyCancelProposalHandler, // Removed in v0.50+
			},
		),
...
func NewSimApp(
	...
	// streaming service removed in v0.50+
	...
	// Note: ParamStore interface has changed in LSM version, commenting out for now
	...
	app.HalvingKeeper = halving.NewKeeper(
		appCodec,
		runtime.NewKVStoreService(keys[halving.StoreKey]),
		app.ParamsKeeper.Subspace(halving.DefaultParamspace),
		app.MintKeeper, app.AccountKeeper,
		authtypes.NewModuleAddress(govtypes.ModuleName).String(),
	)
	...
	*/
	...
	govKeeper.SetLegacyRouter(govRouter) // Removed in v0.50+
	...
	evidenceKeeper := evidencekeeper.NewKeeper(
		...
		addresscodec.NewBech32Codec(Bech32Prefix), runtime.ProvideCometInfoService(),
	)
	...
	app.mm = module.NewManager(
		genutil.NewAppModule(
			app.AccountKeeper, app.StakingKeeper, app, txConfig,
		),
		...
		upgrade.NewAppModule(app.UpgradeKeeper, addresscodec.NewBech32Codec(Bech32Prefix)),
...
func (app *SimApp) AutoCliOpts() autocli.AppOptions {
	...
	for name, m := range app.mm.Modules {
		if mod, ok := m.(appmodule.AppModule); ok {
			modules[name] = mod
		}
	}
